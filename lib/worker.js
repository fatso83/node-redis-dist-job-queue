...
Worker.prototype.processOneItem = function(clientObj, cb) {
  ...
  function goForIt(err, jobListValue, cb) {
    ...
    function onSetResult(err, result) {
      ...
      // already being processed. We need to remove the item from processing
      // queue and then GTFO
      if (!result) {
        var args = [self.processingQueueKey, 1, jobListValue];
        redisClient.send_command('lrem', args, function(err) {
          if (err) {
            self.emit('error', err);
          }
          cb();
